...
def plot_trend_line(
    trends_yearly: pd.DataFrame,
    out_path: Path,
    title: str = "Google Trends: 'machine learning' Search Interest",
):
    if trends_yearly is None or trends_yearly.empty:
        print("[skip] No trends data")
        return
    plt.figure(figsize=(14, 9))
    plt.plot(trends_yearly["year"], trends_yearly["interest"], marker="o")
    plt.title(title, fontsize=20, pad=18)
    plt.xlabel("Year", fontsize=16)
    plt.ylabel("Interest (index 0–100)", fontsize=16, labelpad=8)
    plt.tight_layout()
...
def run_udemy_lead_lag_regression(udemy_share: pd.DataFrame, trends_yearly: pd.DataFrame, out_path: Path) -> float:
    ...
    plt.xlabel("Prior-year Google search interest (index 0–100)")
    plt.ylabel("Δ Share")
    plt.tight_layout(); plt.savefig(out_path, bbox_inches="tight"); print("Saved figure:", out_path)
    return float(r2)
...
def main():
    ...
    if wrote_trends and len(trends) > 0:
        ...
        try:
            ...
            plt.title("Google Trends: 'machine learning' Search Interest", fontsize=20, pad=18)
            plt.xlabel("Year", fontsize=16)
            plt.ylabel("Interest (index 0–100)", fontsize=16, labelpad=8)
            plt.tight_layout()
